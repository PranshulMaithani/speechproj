"""
Export Combined Wav2Vec2 to ONNX + INT8 quantization.

Usage:
    python export_combined_onnx.py

Output:
    checkpoints_combined/wav2vec2_combined.onnx         (fp32)
    checkpoints_combined/wav2vec2_combined_quant.onnx   (int8, use this)
"""
...
from train_wav2vec2_combined import BiasedSpeechClassifier

CKPT_DIR = Path("checkpoints_combined")
CKPT_PATH = CKPT_DIR / "wav2vec2_combined_best.pt"
SAMPLE_RATE = 16000
WINDOW_SEC = 5.0
...
def main():
    ...
    print(f"  Read threshold: {checkpoint.get('read_threshold', 0.65)}")
    ...
    onnx_path = CKPT_DIR / "wav2vec2_combined.onnx"
    ...
    quant_path = CKPT_DIR / "wav2vec2_combined_quant.onnx"
