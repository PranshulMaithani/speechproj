...
head on 10-second audio windows.
...
Usage:
    python -m src.models.train_wav2vec2_10sec --config configs/config.yaml
"""
...
def train(cfg: dict):
    ...
    for epoch in range(1, w2v_cfg["num_epochs"] + 1):
        ...
        if val_f1 > best_f1:
            ...
            torch.save({
                ...
                "val_acc": val_acc,
                "config": w2v_cfg,
            }, ckpt_dir / "wav2vec2_10sec_best.pt")
    ...
    with open(ckpt_dir / "wav2vec2_10sec_history.json", "w") as f:
    ...
    # Load best model
    checkpoint = torch.load(ckpt_dir / "wav2vec2_10sec_best.pt", map_location=device)
    ...
    with open(ckpt_dir / "wav2vec2_10sec_results.json", "w") as f:
...
def main():
    parser = argparse.ArgumentParser(description="Train Wav2Vec2 classifier (10-second windows)")
