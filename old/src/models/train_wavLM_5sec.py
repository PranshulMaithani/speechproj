...
LOGGER = logging.getLogger("train_wavlm_5sec")
...
class SpeechClassifier(nn.Module):
    ...
    def __init__(
        ...
        LOGGER.info(f"  Frozen layers: {freeze_layers}, Hidden: {hidden_size}")
...
def train(cfg: dict):
    ...
    logger = setup_logger(ckpt_dir / "wavlm_5sec_train.log")

    best_ckpt_path = ckpt_dir / "wavlm_5sec_best.pt"
    history_path = ckpt_dir / "wavlm_5sec_history.json"
    results_path = ckpt_dir / "wavlm_5sec_results.json"
    ...
    if torch.cuda.is_available():
        ...
        if vram_gb < 10:
            ...
            logger.info(f"Reduced batch size to {batch_size} for {vram_gb:.0f}GB VRAM")
    ...
    logger.info(
        f"Starting training: {w2v_cfg['num_epochs']} epochs, "
    ...
    for epoch in range(1, w2v_cfg["num_epochs"] + 1):
        logger.info(f"{'=' * 60}")
        logger.info(f"Epoch {epoch}/{w2v_cfg['num_epochs']}")
        logger.info(f"{'=' * 60}")
        ...
        if val_f1 > best_f1:
            ...
            logger.info(f"  New best model (F1={val_f1:.4f}) saved: {best_ckpt_path.name}")
        else:
            patience_counter += 1
